...
var VideoChat = {
  ...
  requestMediaStream: function (event) {
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        localVideoText.text('Puedes moverme si quieres');
        setTimeout(() => localVideoText.fadeOut(), 5000);
      })
      .catch((error) => {
        logIt(error);
        logIt(
          'Error conectando con el video de la webcam, por favor comprueba los settings de privacidad'
        );
  ...
  onMediaStream: function (stream) {
    ...
    Snackbar.show({
      text: 'Aquí está tu enlace para compartir esta llamada: ' + url,
  ...
  startCall: function (event) {
    ...
    try {
      gtag('event', 'startCall', {
        event_category: 'videoEvent',
        event_label: '' + roomHash,
        value: 1,
      });
    } catch (e) {}
  ...
  onCandidate: function (candidate) {
    // Update caption text
    captionText.text('Encontrado otro voluntario... conectando');
...
function chatRoomFull() {
  alert(
    'La sala a la que intentas acceder ya está llema. Por favor, comprueba que no tienes el video abierto en otra pestaña o tab, o prueba con un nuevo enlace.'
  );
...
function pauseVideo() {
  ...
  if (videoIsPaused) {
    ...
    videoButtonText.innerText = 'Pausar Video';
  }
}
// End pause Video
...
function swap() {
  ...
  if (!VideoChat.connected) {
    alert('Tienes que estar en una llamada para poder compartir la pantalla.');
    return;
  }
  ...
  if (mode === 'camera') {
    ...
    Snackbar.show({
      text:
        'Por favor acepta compartir la plantalla. Haz click en la imagen de arriba y pulsa compartir.',
      width: '400px',
      pos: 'bottom-center',
      actionTextColor: '#616161',
      duration: 50000,
    ...
    navigator.mediaDevices
      ...
      .then(function (stream) {
        ...
        swapText.innerText = 'Compartir Webcam';
    ...
    navigator.mediaDevices
      ...
      .then(function (stream) {
        ...
        swapText.innerText = 'Compartir Pantalla';
        switchStreamHelper(stream);
      });
  }
}
...
function requestToggleCaptions() {
  ...
  if (!VideoChat.connected) {
    alert('Tienes que estar conectado a otro usuario para activar los Subtítulos');
    return;
  }
  if (receivingCaptions) {
    captionText.text('').fadeOut();
    captionButtontext.text('Empezar Subtítulos');
    receivingCaptions = false;
  } else {
    Snackbar.show({
      text:
        'Es una funcionalidad experimental. Para que funcione, los dos usuarios de la llamada tienen que usar Chrome',
      width: '400px',
      pos: 'bottom-center',
      actionTextColor: '#616161',
      duration: 10000,
    });
    captionButtontext.text('Apagar Subtítulos');
  ...
  VideoChat.recognition.onresult = (event) => {
    ...
    for (let i = event.resultIndex, len = event.results.length; i < len; i++) {
      ...
      if (event.results[i].isFinal) {
        ...
        dataChanel.send(
          'cap:' + interimTranscript.substring(interimTranscript.length - charsToKeep)
        );
      }
    }
  };
...
function recieveCaptions(captions) {
  ...
  if (captions === 'notusingchrome') {
    alert(
      'El otro participante en la llamada tiene que usar Chrome para que funcionen los Subtítulos en tiempo real. Hemos apagado los subtítulos.'
    );
    receivingCaptions = false;
    captionText.text('').fadeOut();
    captionButtontext.text('Activar Subtítulos');
    return;
  }
...
function togglePictureInPicture() {
  if ('pictureInPictureEnabled' in document || remoteVideoVanilla.webkitSetPresentationMode) {
    ...
    alert('Picture in picture no funciona en tu navegador. ¿Por qué no usas Chrome o Safari?');
  }
}
//Picture in picture
...
function startUp() {
  ...
  // Set tab title
  document.title = 'Adopta Un Abuelo - ' + url.substring(url.lastIndexOf('/') + 1);
  ...
  Snackbar.show({
    text: 'Por favor habilita el acceso a la Webcam y el Micrófono',
    actionText: 'Enséñame cómo',
    width: '455px',
    pos: 'top-right',
    actionTextColor: '#616161',
    duration: 50000,
  ...
  captionText.text('Esperando a que otro voluntario se conecte a la llamada...').fadeIn();
